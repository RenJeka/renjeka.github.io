...
:heavy_exclamation_mark: В случае удаления браузера, либо сбоя системы данные могут удалиться. 
...
--------
###### Обработчик при клике на строку
...
При выборе строки фиксируется текущий объект, на который указывает строка, и при нажатии кнопки **изменить**,  **удалить** или **очистить поля** срабатывает обработчик, который запускает соответствующие методы. 
...
- Создать форму в html-файле:
  - Задать форме атрибут **`novalidate`**
  - добавить в форму поле с метаданными 
  ```html
  <input type="hidden" ...> 
  ```
  ...
  - К полям, которые обязательны для заполнения  необходимодобавить атрибут `required`
